...
class TimeObject {
   ...
   * @param {Number} [seconds = 0] - The value to instantiate this TimeObject
   *    with, in seconds.
   */
   ...
   * @return {TimeObject} - The TimeObject passed in as an argument.
   */
  static increment(t) {
    t.raw++;
   ...
   * @return {string} - The formatted time sting.
   */
   ...
   * @return {{h: number, m: number, s: number}} - An HMS object.
   */
   ...
   *    (hh:mm:ss or mm:ss).
   * @return {number} - The parsed time string represented as seconds.
   */
  static parseSeconds(timeString) {
    const timeParts = timeString.split(':');
    if (timeParts.length === 3) {
      return parseInt(timeParts[0] * 3600, 10) +
